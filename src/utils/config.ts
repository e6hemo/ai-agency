...
import { encryptConfigSecretsDeep, decryptConfigSecretsDeep } from './secureStore.js'
...
function saveConfig<A extends object>(
  ...
  const finalConfigToWrite = encryptConfigSecretsDeep(filteredConfig)
  
  // Write config file with secure permissions - mode only applies to new files
  writeFileSyncAndFlush_DEPRECATED(
    file,
    jsonStringify(finalConfigToWrite, null, 2),
    {
      encoding: 'utf-8',
      mode: 0o600,
    },
...
function getConfig<A>(
  ...
  try {
    ...
    try {
      ...
      const rawConfig = jsonParse(stripBOM(fileContent))
      const parsedConfig = decryptConfigSecretsDeep(rawConfig)
      return {
        ...createDefault(),
        ...parsedConfig,
      }
