...
import {
  ...
  deleteApiKey,
  migrateKeyToVault,
} from './secureStorage/apiKeyVault.ts'
...
/**
 * Derive the vault profile ID from a profile file path.
 * Uses a stable, human-readable key like "openai@/home/user/project".
 */
function getVaultProfileId(filePath: string, envKeyName: string): string {
  return `openclaude:${envKeyName}@${filePath}`
}
...
export function loadProfileFile(options?: ProfileFileLocation): ProfileFile | null {
  ...
  try {
    ...
    // ── Keychain Vault: resolve VAULTED: sentinels back to real keys ─────────
    const resolvedEnv: ProfileEnv = { ...parsed.env }
    // Only the string-valued API key fields are ever vaulted
    type VaultableKey = 'OPENAI_API_KEY' | 'CODEX_API_KEY' | 'GEMINI_API_KEY'
    const vaultedKeys: VaultableKey[] = [
      'OPENAI_API_KEY',
      'CODEX_API_KEY',
      'GEMINI_API_KEY',
    ]

    for (const envKey of vaultedKeys) {
      const stored: string | undefined = resolvedEnv[envKey]
      if (typeof stored === 'string' && stored.startsWith('VAULTED:')) {
        const vaultId = stored.slice('VAULTED:'.length)
        const realKey = loadApiKey(vaultId)
        if (realKey) {
          resolvedEnv[envKey] = realKey
        } else {
          // Vault entry missing — remove sentinel so caller knows key is absent
          delete resolvedEnv[envKey]
        }
      }
    }
...
export function saveProfileFile(
  ...
  // ── Keychain Vault: extract API keys before writing to disk ──────────────
  const envToWrite = { ...profileFile.env }

  // The SECRET_ENV_KEYS we protect in the vault
  const keysToVault: Array<keyof ProfileEnv> = [
    'OPENAI_API_KEY',
    'CODEX_API_KEY',
    'GEMINI_API_KEY',
  ]

  for (const envKey of keysToVault) {
    const rawKey = envToWrite[envKey]
    if (rawKey) {
      const vaultId = getVaultProfileId(filePath, envKey)
      const saved = saveApiKey(vaultId, rawKey)
      if (saved) {
        // Replace with a sentinel so the file never holds the real key
        envToWrite[envKey] = `VAULTED:${vaultId}` as string
      }
      // If vault save fails we leave the key in the file (safe fallback)
    }
  }

  const fileToWrite: ProfileFile = {
    ...profileFile,
    env: envToWrite,
  }

  writeFileSync(filePath, JSON.stringify(fileToWrite, null, 2), {
    encoding: 'utf8',
    mode: 0o600,
  })
  return filePath
...
/**
 * Remove an API key from the OS Keychain for a given profile file.
 * Call this when unregistering a provider profile.
 */
export function deleteProfileApiKey(
  envKeyName: 'OPENAI_API_KEY' | 'CODEX_API_KEY' | 'GEMINI_API_KEY',
  options?: ProfileFileLocation,
): boolean {
  const filePath = resolveProfileFilePath(options)
  const vaultId = getVaultProfileId(filePath, envKeyName)
  return deleteApiKey(vaultId)
}

/**
 * Migrate an existing plaintext profile file: moves any exposed API key
 * into the OS Keychain in-place, rewriting the file.
 * Safe to call multiple times (no-op if already vaulted).
 */
export function migrateProfileFileToVault(options?: ProfileFileLocation): boolean {
  const existing = loadProfileFile(options)
  ...
  for (const envKey of keysToVault) {
    const rawKey = existing.env[envKey]
    if (!rawKey || rawKey.startsWith('VAULTED:')) continue
    const vaultId = getVaultProfileId(filePath, envKey)
    if (migrateKeyToVault(vaultId, rawKey)) {
      didMigrate = true
    }
  }

  if (didMigrate) {
    saveProfileFile(existing, options)
  }
